...
parser.add_argument('--imgs_train_path', '-it', type=str,
                    default=f'dataset/{dataset_name}/ImageTr', help='imgs train data path.')
parser.add_argument('--labels_train_path', '-lt', type=str,
                    default=f'dataset/{dataset_name}/LabelTr', help='labels train data path.')
parser.add_argument('--imgs_val_path', '-iv', type=str,
                    default=f'dataset/{dataset_name}/ImageVal', help='imgs val data path.')
parser.add_argument('--labels_val_path', '-lv', type=str,
                    default=f'dataset/{dataset_name}/LabelVal', help='labels val data path.')
parser.add_argument('--imgs_test_path', type=str,
                    default=f'dataset/{dataset_name}/ImageTs', help='imgs val data path.')
parser.add_argument('--labels_test_path', type=str,
                    default=f'dataset/{dataset_name}/LabelTs', help='labels val data path.')
parser.add_argument('--csv_dir_train', '-ct', type=str,
                    default=f'dataset/{dataset_name}/csv/train.csv',
                    help='labels val data path.')
parser.add_argument('--csv_dir_val', '-cv', type=str,
                    default=f'dataset/{dataset_name}/csv/val.csv', help='labels val data path.')
parser.add_argument('--csv_dir_test', type=str,
                    default=f'dataset/{dataset_name}/csv/test.csv', help='labels val data path.')
...
parser.add_argument('--workers', default=1, type=int, help='batchsize')
parser.add_argument('--lr', default=0.0001, type=float, help='learning rate')
parser.add_argument('--warm_epoch', '-w', default=0, type=int, help='end epoch')
parser.add_argument('--end_epoch', '-e', default=10, type=int, help='end epoch')
...
res = (224, 224)  # Ensure this is defined, e.g., res = (width, height)
df_train = pd.read_csv(os.path.join(os.getcwd(), args.csv_dir_train))
...
train_imgs = [os.path.join(train_imgs, i) for i in df_train['image_name']]
train_masks = [os.path.join(train_masks, i.replace('.jpg', '_segmentation.png')) for i in df_train['image_name']]
val_imgs = [os.path.join(val_imgs, i) for i in df_val['image_name']]
val_masks = [os.path.join(val_masks, i.replace('.jpg', '_segmentation.png')) for i in df_val['image_name']]

# Define a function to load and resize images
def load_and_resize_image(image_path, resize_shape):
    img = cv2.imread(image_path)
    if img is not None:
        img_resized = cv2.resize(img, resize_shape)
        return img_resized
    else:
        raise ValueError(f"Image at {image_path} could not be loaded or does not exist.")

# Apply loading and resizing to images and masks
imgs_train = [load_and_resize_image(i, res)[:, :, ::-1] for i in train_imgs]  # Converting BGR to RGB
masks_train = [load_and_resize_image(i, res)[:, :, 0] for i in train_masks]  # Assuming masks are grayscale
imgs_val = [load_and_resize_image(i, res)[:, :, ::-1] for i in val_imgs]
masks_val = [load_and_resize_image(i, res)[:, :, 0] for i in val_masks]
