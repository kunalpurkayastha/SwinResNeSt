...
parser.add_argument('--csv_dir_test', '-cvt', type=str,
                    default=f'dataset/{dataset_name}/csv/test.csv',
...
def visualize_predictions(original_image, ground_truth_mask, predicted_mask, idx):
    ...
    plt.tight_layout()
    plt.savefig(f"{model_savedir}/segmentation_result_{idx}.png")
    plt.show()
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_dl):
            ...
            visualize_predictions(inputs[0].cpu().numpy().transpose(1, 2, 0),
                                  targets[0].cpu().numpy(),
                                  predicted[0].cpu().numpy(), batch_idx)
